  ...
  return (
    <main className="min-h-screen bg-[#f7f7f5] px-5 pb-20 pt-32 md:px-10">
      <div className="mx-auto max-w-7xl">
        <p className="text-sm font-semibold uppercase tracking-[0.18em] text-black/40">Explore</p>
          ...
          {subjects.map(([name, description, id], index) => (
            ...
            >
              <div className="flex items-start justify-between">
                ...
                <ArrowUpRight size={20} className="text-black/25 transition group-hover:text-black" />
              </div>

              <div className="pb-8 pt-16">
                <h2 className="text-3xl font-semibold tracking-tight">{name}</h2>
                <p className="mt-3 max-w-md leading-7 text-black/45">{description}</p>
              </div>
            </Link>
          ))}
        </div>
